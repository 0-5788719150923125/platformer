...
output "config" {
  description = "Ioshare configuration summary"
  value = {
...
output "ansible_bucket_request" {
  ...
  value = {
    purpose     = "ioshare-ansible-playbooks"
    description = "Ansible playbooks for Ioshare deployment"
    prefix      = "ioshare"
  }
}
...
# Export EKS deployment-tenant pairs (for root module's ioshare_urls output)
