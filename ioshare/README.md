...
The Ioshare module orchestrates infrastructure and application deployments for the Ioshare medical imaging platform. It supports multi-tenant deployments with both EC2 and EKS compute options.
...
```
State Fragment → Ioshare Module → Infrastructure Requests → Storage/Compute Modules
                                                                         ↓
                                                                 Creates Resources
                                                                         ↓
Root main.tf ← Endpoints/Outputs ← Module Outputs
     ↓
Ioshare Module (receives endpoints for application configuration)
     ↓
...
- Ioshare workload SG with RDS, ElastiCache, EFS access
...
| `config` | Ioshare configuration from state fragment |
...
1. **Request Generation**: Ioshare generates infrastructure requests
...
4. **Endpoint Return**: Root passes storage outputs back to ioshare
...
**RDS Clusters**:
- `{namespace}-ioshare-{tenant}-services` (database: `v3s`)
- `{namespace}-ioshare-{tenant}-storage` (database: `imagedb`)

**ElastiCache Clusters**:
- `ioshare-{tenant}-services-cache` (Valkey)
- `ioshare-{tenant}-storage-cache` (Valkey)
- `ioshare-{tenant}-memcached` (Memcached)

**S3 Buckets**:
- `{tenant}-dev-ioshare-images-{unique-suffix}`
...
terraform plan -var="state_fragment=ioshare-poc"
