...
variable "config" {
  ...
  type = map(object({
    ...
    reboot       = optional(any)              # Force reboot: true = HARD stop/start this class's instances on every apply, false/unset = off, or a "<nonce>" string for a one-shot reboot when the value changes. Uses stop-instances --force (works on OOM-hung boxes); the ansible-controller build fires AFTER the reboot completes. Consumed at the root (-> configuration-management), not by this module.
    swap_size    = optional(number, 0)        # Swap file size in GB (0 = no swap). Useful for memory-constrained instances.
    zram_size    = optional(number, 0)        # zram (compressed RAM) swap size in GB (0 = none). Absorbs transient memory spikes far faster than disk swap; typically holds 2-3x its size in compressed pages. Takes priority over the disk swapfile.
