# IOPACS Module
# Domain orchestration for IOPACS medical imaging PACS deployments
    ...
    for deploy_name, config in var.config : [
      for tenant_code in lookup(var.tenants_by_deployment, deploy_name, []) : [
        for bucket_config in config.s3 : {
          ...
          description = "IOPACS ${bucket_config.purpose} for ${deploy_name}/${tenant_code}"
...
resource "aws_ssm_parameter" "maestro_deploy_password" {
  ...
  name  = "/${var.namespace}/iopacs/${each.key}/maestro-deploy-password"
  type  = "SecureString"
...
resource "aws_ssm_parameter" "maestro_ssh_privkey" {
  ...
  name  = "/${var.namespace}/iopacs/${each.key}/ssh-privkey"
  type  = "SecureString"
...
resource "aws_ssm_parameter" "maestro_ssh_pubkey" {
  ...
  name  = "/${var.namespace}/iopacs/${each.key}/ssh-pubkey"
  type  = "String"
