...
locals {
  ...
  # Check if ioshare/iopacs deployments declare applications in their compute config
  # Ioshare: deployment model  -  each deployment has its own compute object
  ioshare_has_apps = anytrue([
    for name, deploy in try(var.service_configs["ioshare"], {}) :
    length(try(deploy.compute.applications, [])) > 0
  ])
  # Iopacs: deployment model  -  each deployment has its own compute map
  iopacs_has_apps = anytrue(flatten([
    for _, deploy in try(var.service_configs["iopacs"], {}) : [
      for _, class_config in try(deploy.compute, {}) :
      length(try(class_config.applications, [])) > 0
    ]
  ]))

  # Check if any ioshare deployment uses EKS (generates Helm applications)
  ioshare_eks_enabled = (
    contains(keys(var.service_configs), "ioshare") &&
    anytrue([
      for name, deploy in var.service_configs["ioshare"] :
  ...
  applications_needs_storage = local.compute_has_applications || local.ioshare_eks_enabled || local.ioshare_has_apps || local.iopacs_has_apps || local.standalone_applications_exist
  ...
  module_requirements = {
    storage = (
      ...
      contains(keys(var.service_configs), "ioshare") ||       # Ioshare needs RDS+ElastiCache+S3
      contains(keys(var.service_configs), "iopacs") ||        # IOPACS needs RDS+S3
      contains(keys(var.service_configs), "iorchestrator") || # IOrchestrator needs RDS+S3
    ...
    compute = (
      ...
      # Auto-enable when ioshare deployments have compute definitions
      anytrue([
        for _, deploy in try(var.service_configs["ioshare"], {}) :
        try(deploy.compute, null) != null
      ]) ||
      # Auto-enable when iopacs deployments have compute definitions
      anytrue([
        for _, deploy in try(var.service_configs["iopacs"], {}) :
        try(deploy.compute, null) != null
      ]) ||
      # Auto-enable when iorchestrator exists (needs ECS clusters)
      contains(keys(var.service_configs), "iorchestrator") ||
    ...
    configuration_management = (
      ...
      local.ioshare_has_apps ||
      local.iopacs_has_apps ||
    ...
    ioshare       = contains(keys(var.service_configs), "ioshare")
    iopacs        = contains(keys(var.service_configs), "iopacs")
    iorchestrator = contains(keys(var.service_configs), "iorchestrator")

    # Applications auto-enables when any compute classes declare applications,
    # when ioshare with EKS deployment exists (generates Helm applications),
    # or when standalone applications are declared directly under services.applications
    applications = (
      local.compute_has_applications ||
      local.ioshare_eks_enabled ||
      local.ioshare_has_apps ||
      local.iopacs_has_apps ||
      local.standalone_applications_exist ||
