...
This module provisions cloud-native infrastructure for the IOrchestrator platform, which runs as a set of containerized services (SaaSApp, CoreApps, SaaS Router) on ECS Fargate with an MSSQL backend.
...
```bash
terraform apply -var-file=terraform.tfvars.iorchestrator

# Get ALB URL
terraform output iorchestrator_urls

# Test routing (will return 403 until SQL Server tenant records exist)
ALB_URL=$(terraform output -json iorchestrator_urls | jq -r '.dev1')
...
```
State Fragment -> IOrchestrator Module -> Infrastructure Requests -> Storage Module
                         |                                                     |
...
| `config` | IOrchestrator deployment configurations from state fragment |
...
```bash
# Use the iorchestrator-specific tfvars
terraform plan -var-file=terraform.tfvars.iorchestrator
terraform apply -var-file=terraform.tfvars.iorchestrator
...
2. **Request Generation**: IOrchestrator emits `rds_instance_requests` and `bucket_requests`
...
| SSM Parameter | `/{namespace}/iorchestrator/{deployment}/deployment-context` |
