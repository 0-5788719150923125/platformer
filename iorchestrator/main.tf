...
# Domain orchestration for IOrchestrator (IO Cloud / SaaSApp) deployments
...
resource "aws_cloudwatch_log_group" "ecs" {
  ...
  tags = {
    ...
    ManagedBy  = "platformer-iorchestrator"
  }
}
...
resource "aws_ecs_task_definition" "main" {
  ...
  execution_role_arn       = var.access_iam_role_arns["iorchestrator-ecs-execution"]
  task_role_arn            = var.access_iam_role_arns["iorchestrator-ecs-task"]
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = concat(
        ...
        each.value.service == "saasapp" ? [
          { name = "BOOTSTRAP_AWS_ASSUME_ROLE_ARN", value = var.access_iam_role_arns["iorchestrator-ecs-bootstrap"] }
        ] : [],
...
resource "aws_ssm_parameter" "deployment_context" {
  ...
  name = "/${var.namespace}/iorchestrator/${each.key}/deployment-context"
  type = "String"
  value = jsonencode({
    deployment = each.key
    namespace  = var.namespace
...
resource "aws_ssm_parameter" "application_role_arn" {
  ...
  type  = "String"
  value = var.access_iam_role_arns["iorchestrator-ecs-app"]
  ...
  bucket_requests = flatten([
    for deploy_name, config in var.config : [
      for bucket_config in config.s3 : {
        ...
        description    = "IOrchestrator ${bucket_config.purpose} for ${deploy_name}"
